...
def get_cardinal_directions(current):
    """Assign North / East / South / West based on marker positions in image."""
    if len(current) != 4:
        return {}

    cents = {mid: np.array(d["centroid"]) for mid, d in current.items()}
    by_y = sorted(cents.items(), key=lambda x: x[1][1])  # small y = top
    by_x = sorted(cents.items(), key=lambda x: x[1][0])  # small x = left
...
def compute_heading_error(current, reference, directions):
    """Calculate yaw (heading) error in degrees using North marker as reference."""
    ...
    # Yaw from rotation matrix (Z-axis rotation)
...
# CHECK MODE – Landing Guidance Active
if mode == "check":
    ...
    if reference is None:
        ...
        while True:
            ...
            if heading_error is not None:
                ...
                # Visual rotation indicator (curved arrow)
                h, w = frame.shape[:2]
                center = (w // 2, h // 2)
                radius = 80
                start_angle = 90  # top
                end_angle = 90 + heading_error * 3  # exaggerate for visibility
