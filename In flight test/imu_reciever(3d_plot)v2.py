...
# History for static graph (time, posX, posY, posZ)
...
def fetch_phyphox_data():
    ...
    try:
        ...
        lin_acc = raw_acc - np.array([0.0, 0.0, -9.81])
        ...
        # Save to history for static graph
...
ax.set_ylabel('Y (m)')
ax.set_zlabel('Z (m)')
ax.set_title("Live 3D IMU Position Map (100 m cube)")
...
def update(frame):
    ...
    fig.suptitle(f"Live 3D IMU Position | Current: ({position[0]:.2f}, {position[1]:.2f}, {position[2]:.2f}) m")
...
def show_static_graph():
    ...
    plt.plot(seconds, pos_x, label='X Position', color='r')
    plt.plot(seconds, pos_y, label='Y Position', color='g')
    plt.plot(seconds, pos_z, label='Z Position', color='b')
    plt.xlabel('Time (seconds)')
    plt.ylabel('Position (meters)')
    plt.title('Position History After Live Session')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("position_history_graph.png")
    plt.show()
    print("Static graph saved as 'position_history_graph.png'")

# Handle exit (Ctrl+C or close window)
def on_exit(sig=None, frame=None):
    print("\nExiting live plot... Generating static graph.")
    show_static_graph()
    sys.exit(0)
